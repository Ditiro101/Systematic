...
let buildTable=function(tmp)
{
	...
	let productUnitPriceEntry=$("<td></td>").addClass("text-right py-3").text("R"+productData[tmp]["PRICE"]);
	let productTotal=parseFloat(productData[tmp]["QUANTITY"]).toFixed(2)*parseFloat(productData[tmp]["PRICE"]).toFixed(2);
