...
let buildTable=function(tmp,assArr,assPArr)
{
	...
	let formEntry="<form action='finalise_delivery.php' method='POST'><input type='hidden' name='ass' value='"+JSON.stringify(saleArr)+"'>"+"<input type='hidden' name='assP' value='"+JSON.stringify(saleProductsArr)+"'>"+"<button class='btn btn-icon btn-2 btn-primary btn-sm' type='submit'><span class='btn-inner--icon'><i class='fas fa-truck'></i></span><span class='btn-inner--text'>Make Delivery</span></button>";
